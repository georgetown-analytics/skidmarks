...
stops = val.loc[val['Increment Traveled (feet)'] <= 3].index
...
if len(stops) < 1:
	...
	#iterate through stops and flag start and end of stops in the dataframe

	maneuvers = 0

	df['start'][stops[0]] = 1	#the first index must be the start of the first turn

	for i in range(1, len(stops)-1):
		#print i
		if stops[i] - 1 > stops[i-1]:
			df['start'][stops[i]] = 1	#the current index and last index are more than 1 second apart so this must be the start of a new turn
			df['end'][stops[i-1]] = 1	#the last index must also be the end of the last turn since we are beginning a new turn

	df['end'][stops[len(stops)-1]] = 1	#the last index must be the end of the last turn
...
'''
Calculating stops and stops: Use the rolling window.  Divide "Change in Direction per s" by "Velocity (mph)" or vice versa to get a figure.
