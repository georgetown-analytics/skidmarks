...
def main(argv):
    ...
    if "locations" in data and len(data["locations"]) > 0:
        ...
        if args.format == "csv":
            f_out.write("Date,Time,Location, Latitude, Longitude,Accuracy\n") #Activity,#Activity Confidence\n")
            for item in items:
                f_out.write(datetime.fromtimestamp(int(item["timestampMs"]) / 1000).strftime("%Y-%m-%d"))
                f_out.write(",")
                f_out.write(datetime.fromtimestamp(int(item["timestampMs"]) / 1000).strftime("%H:%M:%S"))
                f_out.write(",")
                f_out.write("%s " % (item["latitudeE7"] / 10000000))
                f_out.write(",")
                f_out.write("%s " % (item["longitudeE7"] / 10000000))
                f_out.write(",")
                f_out.write("%s " % (item["accuracy"]))
                f_out.write(",")

                for activity in item[activitys]:
                    f_out.write()

                #f_out.write("%s " % (item["type"]))
                #f_out.write(",")
                #f_out.write("%s \n" % (item["confidence"]))
